...
export function MedicationCard({ insulin, oral, type }: MedicationCardProps) {
    ...
    const renderScheduleList = (items: MedicationScheduleData['schedules'] | undefined, medType: 'insulin' | 'oral_hypoglycemic') => {
        const sortedItems = [...(items || [])].sort((a, b) => {
            return TIME_PERIODS.indexOf(a.time_period) - TIME_PERIODS.indexOf(b.time_period);
        });

        if (sortedItems.length === 0) {
            return <p className="text-gray-400 text-sm italic">{t('insulinSchedule.noActiveSchedule')}</p>;
        }
    ...
    if (type === 'both') {
        return (
            <>
                <ProfileSection title={t('steps.insulinSchedule')} icon={Syringe}>
                    <div className="space-y-3">
                        {renderScheduleList(insulin?.schedules, 'insulin')}
                    </div>
                </ProfileSection>
                <ProfileSection title={t('steps.oralMedication')} icon={Pill}>
                    <div className="space-y-3">
                        {renderScheduleList(oral?.schedules, 'oral_hypoglycemic')}
                    </div>
                </ProfileSection>
            </>
        );
    ...
    return (
        <ProfileSection title={title} icon={Icon}>
            <div className="space-y-3">
                {renderScheduleList(
                    type === 'insulin' ? insulin?.schedules : oral?.schedules,
                    type,
                )}
            </div>
        </ProfileSection>
